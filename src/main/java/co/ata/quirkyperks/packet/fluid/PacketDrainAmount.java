...
public class PacketDrainAmount extends PacketFluid {
    ...
    public void touchHandler(TileWarper target, WarpInterface iface, IFluidHandler handler, EnumFacing f) {
        ...
        if(drainStack == null){
            ...
            if(pulled != null && iface.canInterface(ItemWarpCard.getFilters(target.card()), f, EnumInterfaceDirection.In, pulled)
                              && iface.canInterface(ItemWarpCard.getFilters(source.card()), facing, EnumInterfaceDirection.Out, pulled)){
                pulled = handler.drain(amount, true);
                drainStack = pulled;
                drained = drainStack.copy();
                drained.amount = amount - drainStack.amount;
            }
        }else if(iface.canInterface(ItemWarpCard.getFilters(target.card()), f, EnumInterfaceDirection.In, drainStack)){
